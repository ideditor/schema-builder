...
function buildData(options) {
  ...
  const deprecated = require(dataDir + '/deprecated.json');
  validateSchema(dataDir + '/deprecated.json', deprecated, require('../schemas/deprecated.json'));

  const discarded = require(dataDir + '/discarded.json');
  ...
  fs.writeFileSync(distDir + '/translations/en.json', JSON.stringify({ en: { presets: tstrings } }, null, 4));
  fs.writeFileSync(distDir + '/translations/en.yaml', translationsToYAML({ en: { presets: translations } }));
...
function generateTaginfo(presets, fields, deprecated, projectInfo) {
  ...
  if (!projectInfo.name) projectInfo.name = package.name;
  if (!projectInfo.description) projectInfo.description = package.description;
...
if (process.argv[1].indexOf('schema-builder') > -1) {
  buildData();
}
