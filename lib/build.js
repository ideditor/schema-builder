...
const fetchTranslations = require('./translations.js').fetchTranslations;
...
function processData(options, type) {
  ...
  const dataDir = process.cwd() + '/' + options.inDirectory;
  ...
  const interimDir = process.cwd() + '/' + options.interimDirectory;
  ...
  const distDir = process.cwd() + '/' + options.outDirectory;
  ...
  if (options.translOrgId && options.translProjectId) {
...
function generatePresets(dataDir, tstrings, searchableFieldIDs, listReusedIcons) {
  ...
  glob.sync(dataDir + '/presets/**/*.json').forEach(file => {
    ...
    tstrings.presets[id].terms = Array.from(new Set(
      (preset.terms || [])
        .map(t => t.toLowerCase().trim())
        .filter(Boolean)
    )).join(',');
  ...
  if (listReusedIcons) {
    ...
    const iconIDs = Object.keys(icons).sort(function(iconID1, iconID2) {
      return icons[iconID2].length - icons[iconID1].length;
    });

    let reusedIconPresetCount = 0;
    const reusedIconCount = Object.keys(icons).filter(function(iconID) {
        if (icons[iconID].length > reuseLimit) {
            reusedIconPresetCount += icons[iconID].length;
            return true;
        }
        return false;
    }).length;

    if (reusedIconCount > 0) {
      console.log(reusedIconCount + ' icon(s), including (none), are each used more than ' + reuseLimit + ' time(s) across ' + reusedIconPresetCount + ' searchable presets\n');

      iconIDs.forEach(function(iconID) {
        const presetIDs = icons[iconID];
        if (presetIDs.length >= reuseLimit) {
            console.log(iconID + ', ' + presetIDs.length);
            for (let i in presetIDs) {
              console.log('-' + presetIDs[i]);
            }
            console.log('');
         }
      });
    } else {
      console.log(colors.green('No icons are used more than ' + reuseLimit + ' time(s) across ' + reusedIconPresetCount + ' searchable presets\n'));
    }
  }
  ...
  for (let presetId in yamlStrings.presets) {
    ...
    if (keys.length) {
      ...
      if (yamlPreset.aliases) {
        yamlPreset['#name'] += ' | ' + yamlPreset.aliases.join(', ');
      }
