...
export function generateCountryMetadata() {
  /**
   * Contains every group from `country-coder` (such as `419` or `Q46`)
   * mapped to the respective iso1A2 code.
   * @type {{ [group: string]: Set<string> }}
   */
  const groups = {};

  /** Contains every code that `country-coder` recognises */
  const allCodes = new Set();

  for (const border of borders.features) {
    allCodes.add(border.properties.iso1A2);
    allCodes.add(border.properties.country);
    allCodes.add(border.properties.wikidata);
    allCodes.add(border.properties.m49);

    const code = border.properties.iso1A2 || border.properties.country;
    if (code) {
      for (const group of border.properties.groups) {
        if (!/^[a-z]{2}$/i.test(group)) {
  ...
  // loop again to alias m49 IDs and Q-IDs, since they're used interchangably in groups
  for (const border of borders.features) {
    const { wikidata: qId, m49 } = border.properties;
    if (m49 && qId) {
      groups[qId] ||= groups[m49];
      groups[m49] ||= groups[qId];
    }
  }

  return { allCodes, groups };
}
const countryMetadata = generateCountryMetadata();
...
/**
 * this function mutates its argument, and expands any 'groups' in the locationSets
 * to the equivalent ISO 3166-1 codes country codes. This makes it simpler for
 * downstream consumers.
 * @param {{ include?: string[]; exclude?: string[] } | undefined} locationSet
 */
function expandLocationSet(locationSet) {
  /** @param {string} _code */
  function expandCodes(_code) {
    const code = _code.toUpperCase();
    if (!countryMetadata.allCodes.has(code)) {
      // eslint-disable-next-line no-console
      console.warn(`Country coder does not recognise “${code}”`);
    }

    return [...(countryMetadata.groups[code] || [code])];
  }

  if (!locationSet) return;
  locationSet.include &&= locationSet.include.flatMap(expandCodes);
  locationSet.exclude &&= locationSet.exclude.flatMap(expandCodes);
}
...
function generateFields(dataDir, tstrings, searchableFieldIDs) {
  ...
  globSync(dataDir + '/fields/**/*.json', {
    ...
    expandLocationSet(field.locationSet);
